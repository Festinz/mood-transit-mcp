...
- Tags contain at most eight validated music-tag values. Artist seeds must be valid MusicBrainz UUIDs, and counts/popularity bounds are capped.
...
- Weather and public music discovery start concurrently. For semantic/context music discovery, ListenBrainz starts first and MusicBrainz opens after a 175 ms hedge delay only if strict metadata has not already arrived. A strict first batch returns immediately and cancels any already-started MusicBrainz loser so it cannot drain the global rate-limit queue; a merely rankable but broadened batch gets a 2.4-second total hedge window for a slower strict batch before the server proceeds transparently with broadened results. Exact discovery decisions are cached in memory for ten minutes under a SHA-256 request key.
